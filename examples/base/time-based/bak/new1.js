...
CC.requestInterval = function (fn, delay) {
    ...
    function loop() {
        ...
        if(realDelta >= delay) {
            fn.call(null, realDelta, 1000 / realDelta);
...
CC.requestTimeout = function (fn, delay) {
    ...
    function loop() {
        ...
        realDelta >= delay ? fn.call(null, realDelta, 1000 / realDelta) : handler.reqId = requestAnimationFrame(loop);
    };
...
// 运动系数
var motionCoefficient = 1;

var DT = 1000 / 60;

CC.loop = function (opts) {

    var previous = +new Date;
    var accumulateTime = 0;

    CC.requestInterval(function (realDelta, realFps) {
        var current = +new Date;
        var passed = current - previous;
        previous = current;
        accumulateTime += passed;
        while (accumulateTime >= DT) {
            opts.update(realDelta, realFps);
            accumulateTime -= DT;
        }
        opts.render(realDelta, realFps);
    }, 1000 / opts.fps);
}
...
// move(document.getElementById('div7'), 60);
// move(document.getElementById('div8'), 30);
move(document.getElementById('div9'), 10);
